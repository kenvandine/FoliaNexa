...
| [`bot/`](bot/) | `folia-nexa-bot` | Discord bot: `/status`, `/who`, `/leaderboard`, `/request-access`, plus background loops that keep access synced to live Discord role membership (grant/revoke, no operator action needed after the one-time `/request-access`) and announce a channel message whenever a player joins a world. |
