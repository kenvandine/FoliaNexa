...
The `snapshot_schedule`/`snapshot_expiry` LXD-native mechanism above is only ever applied at container **launch** time — nothing re-pushes it on edit, and mgmt never reads LXD's own snapshot list back, so there was no way to browse or restore from it in the dashboard. A separate, mgmt-driven mechanism sits alongside it for that — and unlike that LXD-native one, it's **file-level, not an LXD instance snapshot**: a tar.gz of the world save (`world/`) and `plugins/` (jars included, not just plugin data, so a restore brings back the exact plugin versions that were running at backup time), fetched from `folia-nexa-node`'s own `GET /backup` endpoint over plain HTTP and stored on **mgmt's own disk** (`Settings.world_backups_dir`, one file per backup at `world-backups/<world_name>/<label>.tar.gz`) — never the world's own LXD host or container. This was a deliberate redesign away from an earlier LXD-snapshot-based version of this same feature, after two live incidents traced to LXD's `dir` storage driver having no native copy-on-write support: every snapshot on that driver freezes the whole container for a full rootfs copy, which both blocked on a missing `restricted.snapshots` project permission and, separately, wedged a container in `FREEZING` when a client-side timeout during that freeze caused overlapping snapshot requests. See `mgmt/src/folia_mgmt/world_backups.py` and CLAUDE.md's World backups entry for the incident history — the file-level design sidesteps the storage layer entirely, so it behaves identically regardless of what's backing a host's containers.
...
- `scheduler.run_scheduled_backups`, one of `reconcile()`'s steps (§5), streams a tar.gz off every `backups_enabled` running world's own node agent (`world_backups.fetch_and_store_backup`) once an hour, tracked in a `WorldBackup` row (world name, backup label, `kind="scheduled"`, timestamp, size) — self-gated to hourly by comparing against each world's own most recent row, since `reconcile()` itself ticks every 15s. Every attempt (success or failure) also stamps `World.last_backup_attempt_at`/`last_backup_error`, surfaced directly in the dashboard's Backups section rather than failing silently.
- `scheduler.prune_expired_backups`, the same loop, deletes both the row and the local tarball once older than a week (`BACKUP_RETENTION`), keeping "a week's worth" per world — purely local-disk work now, no host/world resolution needed to find the right file. Applies equally to `kind="manual"` rows below — there's no separate retention tier for an on-demand backup.
...
- `POST /worlds/{name}/backups/{backup_id}/restore` is **admin-only** (stricter than the plain operator-gated ad-hoc snapshot/restore endpoints below, since this is reachable straight from the dashboard by any logged-in operator otherwise). Pushes the stored tar.gz to `{NODE_WORLD_DIR}/.pending-restore.tar.gz` (proven against a *running* container, like every other `push_file` call in this codebase) and restarts the container (`LXDClient.restart_container` — the same call `recover_crashed_worlds` already relies on) — the node agent process fully cycles, and its `main()` extracts the marker over `world/`/`plugins/` before any other staging, on the fresh boot that follows. No new synchronization needed in the node agent for this — a full container restart already gives a clean stop/replace/relaunch boundary. Restoring onto a *different* host works the same way with no special-casing, since this endpoint only ever looks at wherever the world is currently placed, not where the backup was originally taken: place a same-named world on a healthy host first (or migrate an existing one there), then restore.
- Backups living on mgmt's own disk (not a world's host) doubles as the disaster-recovery story for losing the mgmt node itself: `world-backups/`'s flat, deterministic layout is rsync-friendly as-is, so an operator can point a cron/systemd-timer job at it from a separate backup host with no code involved on either side.

The older, LXD-instance-snapshot-based mechanism (`LXDClient.snapshot_container`/`restore_snapshot`, and the ad-hoc `POST /worlds/{name}/snapshot`/`POST /worlds/{name}/restore/{snapshot_name}` endpoints) is kept in the codebase as a lower-level feature — useful on a host whose storage pool actually has native snapshot support (ZFS/btrfs/LVM; see `tools/migrate-storage-to-zfs.sh`) — but **off by default** (`Settings.lxd_snapshot_backups_enabled`). The tracked "time machine" feature above never depends on this flag at all. Independent of the flag, `LXDClient` itself unconditionally refuses to snapshot or restore an instance whose storage pool driver is `dir` (`get_storage_driver_for_instance`) — a hard safety net so the freeze incident that motivated this whole redesign can't recur even if the flag is re-enabled on a host still using `dir`.

Real pytest coverage across `mgmt/tests/test_world_backups.py` (the fetch/store/validate logic against a real local HTTP server standing in for a node agent — success, non-200, a truncated/corrupt stream, connection refused), `mgmt/tests/test_scheduler.py` (hourly-gating and retention-pruning in isolation, including the mid-flight-disable and hard-deleted-world race guards), `mgmt/tests/test_worlds.py` (the API endpoints end-to-end through a real reconcile pass, including the ad-hoc endpoints' disabled-by-default 403), and `mgmt/tests/test_lxd_client.py` (the "dir" refusal and the per-container in-flight lock on the kept LXD-snapshot path). Not verified against live infrastructure: a real `folia-nexa-node`'s `/backup`/restore-extraction path against a real running Folia server, and the kept LXD-snapshot path's own `snapshot_container`/`restore_snapshot` calls beyond what CLAUDE.md's World backups entry already documents as confirmed.
...
| `POST` | `/api/v1/worlds/{name}/snapshot` | On-demand LXD instance snapshot — off by default, §6A |
| `POST` | `/api/v1/worlds/{name}/restore/{snapshot}` | Roll back to an LXD instance snapshot — off by default, §6A |
